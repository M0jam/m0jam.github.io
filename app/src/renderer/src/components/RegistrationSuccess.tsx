...
export function RegistrationSuccess({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center bg-slate-950 transition-opacity duration-500 ${step >= 3 ? 'opacity-0' : 'opacity-100'}`}>
        <div className="text-center px-4">
            ...
            <div className={`transition-all duration-700 delay-300 transform ${step >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 mb-3">{t('registration.welcome')}</h1>
                <p className="text-lg text-slate-400 mb-8 max-w-md mx-auto">{t('registration.success')}</p>
                
                <button 
                    onClick={handleContinue}
                    className={`group relative inline-flex items-center gap-2 px-8 py-3 bg-primary-600 hover:bg-primary-500 text-white rounded-full font-medium transition-all duration-500 transform ${step >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 delay-500'}`}
                >
        ...
        <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
             ...
             <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary-500/10 rounded-full blur-[100px] animate-pulse"></div>
